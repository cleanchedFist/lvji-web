...
const UploadContractModal = forwardRef<UploadContractModalRef>((props, ref) => {
  ...
  const onLawyerChange = (id: number) => {
    setLawyerId(id);
  };
  ...
  function handleUploadDir() {
    ...
    const fileRequest = fileList.map((item) => {
      const params = { model, versionId: '1.0.0', reviewerId: lawyerId };
      return uploadContract(item as unknown as File, params);
    });
  ...
  function handleUploadVersion() {
    ...
    const fileRequest = uploadVersion(file as unknown as File, {
      dirId: `${dirId}`,
      model,
      versionId: nextVersionName,
      reviewerId: lawyerId,
    });
    fileRequest
      .then((res) => {
    ...
    <>
      ...
      >
        <div className="py-4">
          ...
          {initialState?.isUserRole ? <LawyerSelector onChange={onLawyerChange} /> : <></>}
