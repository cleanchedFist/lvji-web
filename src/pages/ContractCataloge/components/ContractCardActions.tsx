...
const handleViewContract = (data: API.CatalogeCardProps) => {
    history.push(`/clm/contract/view/${data.latestFileId}`);
}
...
export default function ContractCardActions(_: any, data: API.CatalogeCardProps) {
    ...
    return [
        ...
        <Button key="delete" onClick={() => { handleDeleteClick(data) }}>删除</Button>,
        <Button key="upload" type="primary" onClick={() => { contractVersionsContext.onUploadDVersionBtnClick(data.id) }}>上传新版本</Button>,
    ]
}
